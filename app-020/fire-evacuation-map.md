...
2. **平面绘制**（毫米坐标，吸附 0.1m）：多边形画房间（办公/商业/仓库/病房/走道/其他六种用途）与走道，可拖动整体平移；六类设施点位（灭火器/消火栓/疏散指示灯/应急照明/安全出口/喷淋）点击放置，编号自动生成（`楼栋-楼层-类型-序号`，如 `A-3F-EX-01`；楼栋未编号时省略楼栋段为 `3F-EX-01`；查重取本层同类型最大序号 +1，删中间一台再加不撞号，同层唯一）。楼栋编号可在楼栋页修改，修改后本楼全部设施编号级联跟随；历史数据加载时一次性归一化。
...
- 全楼台账页：跨建筑、跨楼层按设施类型筛选，勾「只看待整改（过期/缺失/损坏）」，按下次应检日期排序，导出 CSV（带 BOM，Excel 直接打开）。台账条数与图上实摆数同口径显示并逐条对账；每行「查看定位」深链 `#/floor/:id?fac=设施id`，在图上居中高亮该设施，不存在悬空行。
...
type Facility = { id: string;
                  ...
                  x: number; y: number; code: string;      // 如 A-3F-EX-01
...
## 10. 验收标准
- 单元测试 **8 个文件 / 77 个用例**全部通过（vitest 2.1.9，`npm test`）：疏散距离 20 组、灭火器覆盖 10 组、检查台账 8 组、编号 16 组、store 回归 13 组、规则切换 4 组、台账行对账 4 组、性能 2 组。- 设施编号：删除中间一台再添加按「最大序号 +1」续号，同层同类型绝不撞号（I7/I8、S11）；编号带楼栋与楼层前缀（`A-3F-EX-01`），两栋楼互不干扰（I9/I10、S12）；楼栋编号事后修改，本楼全部设施编号级联跟随（S13）；旧数据与重号在加载时归一化（I13–I15）。
- 台账对账：按类型筛选的条数等于图上同口径实摆数，且每行都能按楼层 + 设施 id 定位到实物（G1–G3）；同层重号在引擎中判 `FACILITY_CODE_DUPLICATE`（error，可定位）。
